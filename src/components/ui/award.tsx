...
export function Awards({
  ...
  if (variant === "award") {
    return (
      ...
      >
        <svg
          className={cn("absolute inset-0 m-auto opacity-90 pointer-events-none")}
        ...
        <div className="z-10 px-6 py-4 text-center relative max-w-[140px] mx-auto">

          <h1 className={cn("text-[13px] font-black tracking-tight text-foreground drop-shadow-sm leading-tight break-words")}>{title}</h1>

          <div className="bg-accent/60 mx-auto my-1.5 h-[1px] w-16" />

          <h2 className={cn("text-[10px] font-medium text-muted-foreground leading-snug break-words")}>{subtitle}</h2>

          {recipient && (
            <p className={cn("text-accent/90 italic text-[10px] mt-0.5 break-words")}>{recipient}</p>
          )}

          <div className={cn("text-accent text-[10px] font-bold mt-1")}>{date}</div>
        </div>
      </div>
    )
  }
