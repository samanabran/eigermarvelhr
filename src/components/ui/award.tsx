    ...
    return (
      ...
      >
        ...
        <div className="z-10 px-6 py-4 text-center relative max-w-[140px] mx-auto">
          ...
          <h1 className={cn("text-[13px] font-black tracking-tight text-white drop-shadow-md leading-tight break-words")}>{title}</h1>

          <div className="bg-amber-400 mx-auto my-1.5 h-[1px] w-16" />

          <h2 className={cn("text-[10px] font-medium text-amber-100/90 leading-snug break-words")}>{subtitle}</h2>

          {recipient && (
            <p className={cn("text-amber-200/80 italic text-[10px] mt-0.5 break-words")}>{recipient}</p>
          )}

          <div className={cn("text-amber-300 text-[10px] font-bold mt-1")}>{date}</div>
        </div>
      </div>
    )
  }
