...
export function Awards({
  ...
  if (variant === "award") {
    return (
      ...
      >
        <svg
          className={cn("absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-90")}
        ...
        <div className="z-10 px-8 text-center relative">
          ...
          <h1 className={cn("text-base font-black tracking-tight text-white drop-shadow-md leading-tight")}>{title}</h1>

          <div className="bg-amber-400 mx-auto my-1.5 h-[1px] w-24" />

          <h2 className={cn("mb-2 w-48 text-[11px] font-medium text-amber-100/90 leading-snug")}>{subtitle}</h2>

          {recipient && (
            <p className={cn("text-amber-200/80 italic text-[11px]")}>{recipient}</p>
          )}

          <div className={cn("text-amber-300 text-xs font-bold mt-0.5")}>{date}</div>
        </div>
      </div>
    )
  }
